...
for k in range(len(fnames_m)):
    ...
    if Analisis_de_Fourier == 1: 
        ...
        # if fig_fourier or fig2_fourier:
    ...
    if k!=len(fnames_m):
        try:  
            ...
            output_dir_ciclos= os.path.join(output_dir,'ciclos_H_M')
            if not os.path.exists(output_dir_ciclos):
                os.makedirs(output_dir_ciclos)
...
if templog:
    norm = plt.Normalize(temp_m.min(), temp_m.max())# Normalizar sobre el rango de temperaturas
else:
...
if Analisis_de_Fourier==1:
    ...
    if not templog:
        ...
        cbar = plt.colorbar(sm, label='Temperatura (°C)', ax=ax)
        cbar.set_ticks(np.linspace(temp_m.min(), temp_m.max(), num=10))  # Ajustar número de ticks según necesidad
        cbar.set_ticklabels([f'{tick:.1f}' for tick in np.linspace(temp_m.min(), temp_m.max(), num=10)])
    if Ciclo_promedio:
